...
To evaluate how good the two parameter estimators are we will perform residual resampling bootstrap. This works by picking two consecutive $x_i, x_{i+1}$ at random from the time series, and predicting $100$ new data points,

\begin{align}
  x_{i}  ,x_{i+1} , x_2^{*} , x_3^{*},...,x_{99}^{*},x_{100}^{*},x_{101}^{*},x_{102}^{*}. \\
\end{align}
...
We can see that both the variance and the bias is significantly greater for both parameters when the LS estimator is applied. Thus the LA estimator outperforms the LS estimator and is optimal for this data. This can be explained by the fact that the time series is non Gaussian and that the LS estimator is optimal for Gaussian time series. However it does not mean that the LS estimator outperforms the LA estimator on non Gaussian time series, as this is quite a large class of time series.
...
for(i in c(1:1500)){
  ...
  beta_boot_LA <- ARp.beta.est(bootstrapped_sequence_LA,2)$LA
  ...
  #find x_101 using the formula x_101=beta_1*x_100+beta_2*x_99+residual
